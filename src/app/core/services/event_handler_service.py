...
from app.core.event_engine.handlers import event_handler
...
def create_event_handler(event_handler: event_handler.EventHandler, session: ScopedSession, *, plant_id: int | None = None, device_id: int | None = None) -> int:
	...
	handler_information = EventHandlerInformation(device_id=device_id, plant_id=plant_id)
...
def delete_event_handler(event_handler_id: int, session: ScopedSession):
	event_handler = session.get(EventHandlerInformation, event_handler_id)
	...
	try:
		...
		logging.error('Failed to delete event handler')
		logging.exception(e)
		raise e
...
from app.core.services import get_user, create_user, login, get_devices, create_device, get_alerts, edit_user, delete_user, reset_user_password, update_user_password, edit_user_preferences, start_user_reset_password, get_plants, create_plant, verify_verification_code, verify_password_reset_code
	...
	def post(self, body: dict, query: dict):
		try:
			...
			if code_type == VerificationCodeType.password_reset:
				user_id = verify_password_reset_code(body['email'], body['code'], db.session)
			else:
